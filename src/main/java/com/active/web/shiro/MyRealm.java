...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
		...
		if (loginName != null) {
			...
			System.out.println(userId + " 对应的权限为：" + rs.toString());
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		if (userFormMaps.size() != 0) {
			...
			session.setTimeout(7200000);	//手动设置session过期时间
			logger.info("session time out is :"+session.getTimeout());
			logger.info("authenticationInfo is " + authenticationInfo.getPrincipals() + " ; " + authenticationInfo.getCredentials() + " ; " + authenticationInfo.getCredentialsSalt());
