...
public class PooledDataSource implements
	...
	public void forceCloseAll() {
		synchronized (state) {
			...
			for (int i = state.activeConnections
					.size(); i > 0; i--) {
				try {
					org.apache.ibatis.datasource.pooled.PooledConnection conn = state.activeConnections
							.remove(i - 1);
					conn.invalidate();
			...
			for (int i = state.idleConnections
					.size(); i > 0; i--) {
				try {
					org.apache.ibatis.datasource.pooled.PooledConnection conn = state.idleConnections
							.remove(i - 1);
					conn.invalidate();
	...
	protected void pushConnection(
			org.apache.ibatis.datasource.pooled.PooledConnection conn)
			throws SQLException {
		...
		synchronized (state) {
			...
			if (conn.isValid()) {
				if (state.idleConnections
					...
					org.apache.ibatis.datasource.pooled.PooledConnection newConn = new org.apache.ibatis.datasource.pooled.PooledConnection(
							conn.getRealConnection(),
							this);
	...
	private org.apache.ibatis.datasource.pooled.PooledConnection popConnection(
			String username,
			String password)
			throws SQLException {
		boolean countedWait = false;
		org.apache.ibatis.datasource.pooled.PooledConnection conn = null;
		long t = System
				.currentTimeMillis();
			...
			synchronized (state) {
				if (state.idleConnections
					...
					if (state.activeConnections
						...
						conn = new org.apache.ibatis.datasource.pooled.PooledConnection(
								dataSource
										.getConnection(),
								this);
						...
						org.apache.ibatis.datasource.pooled.PooledConnection oldestActiveConnection = state.activeConnections
								.get(0);
	...
	public static Connection unwrapConnection(
		...
		if (Proxy.isProxyClass(conn
			...
			if (handler instanceof org.apache.ibatis.datasource.pooled.PooledConnection) {
				return ((org.apache.ibatis.datasource.pooled.PooledConnection) handler)
						.getRealConnection();
			}
		}
		return conn;
