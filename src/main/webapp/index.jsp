...
<script type="text/javascript">
	...
	function setting(userId){
		...
		pageii = layer.open({
			...
			type : 2,
			area : [ "40%", "80%" ],
			content : rootPath + '/user/setting.shtml?userId='+userId
		});
	}
	$(function() {
		var tb = $("#loadhtml");
		...
		var roleKey="${roleKey}";
		var postPath = rootPath+picture;
		$("#post").attr("src",postPath);
		...
		<header class="bg-dark dk header navbar navbar-fixed-top-xs">
			<div class="navbar-header aside-md">
				<a class="btn btn-link visible-xs"
					...
					src="${ctx}/notebook/notebook_files/logo.png" class="m-r-sm">Listener</a>
			...
			<ul class="nav navbar-nav navbar-right m-n hidden-xs nav-user">
				...
				<li class="dropdown"><a href="index.html#"
					class="dropdown-toggle" data-toggle="dropdown"> <span
						class="thumb-sm avatar pull-left"> <img id="post"
							src="">
					</span> ${userName}
					<%--<b class="caret"></b>--%>
				</a>
					<%--<ul class="dropdown-menu animated fadeInRight">--%>
						...
						<%--</a></li>--%>
						<%--<li><a href="javascript:setting(${userId})">Setting</a></li>--%>
									...
									<ul class="nav">
										<c:forEach var="key" items="${list}" varStatus="s">
											...
											<li <c:if test="${s.index==0}">class="active"</c:if>><a
												...
												</c:if> <c:if test="${s.index==15}">
													<i class="fa fa-book icon"> <b class="bg-info"></b>
													</i>
												</c:if> <c:if test="${s.index==16}">
													<i class="fa fa-th-list icon"> <b class="bg-success"></b>
													</i>
												</c:if>
